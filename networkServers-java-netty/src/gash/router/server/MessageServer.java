...
public class MessageServer {
	...
	private void init(File cfg) {
		...
		System.out.println("----------------------- init(conf): NodeState.getInstance().setState(NodeState.FOLLOWER);");
	}
	...
	private static class StartWorkCommunication implements Runnable {
		...
		public StartWorkCommunication(RoutingConf conf) {
			...
			//John: Comment here with "NodeID FOLLOWER SERVICE STARTED "
			java.util.Date date = new java.util.Date();
			gash.router.util.Logger.DEBUG("--------StartWorkCommunication():--------------"
